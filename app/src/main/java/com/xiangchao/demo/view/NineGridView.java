package com.xiangchao.demo.view;
...
public class NineGridView extends ViewGroup {
    ...
    private int singleImageSize = 360;              // 单张图片时的最大大小,单位dp
    ...
    private List<ImgInfo> mImageInfo;
...
//                gridWidth = gridHeight = (totalWidth - gridSpacing * (columnCount - 1)) / columnCount;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            if (mAdapter != null) {
                mAdapter.onDisplayImage(getContext(), childrenView, mImageInfo.get(i).url);
            }
    ...
    public void setAdapter(@NonNull NineGridViewAdapter adapter) {
        mAdapter = adapter;
        List<ImgInfo> imageInfo = adapter.getImageInfo();
        ...
        //判断是否为视频格式
        if (adapter.getImageInfo().size() == 1 && adapter.getImageInfo().get(0).isVideo) {
            View child = getChildAt(0);
            if (child instanceof NineGridViewWrapper) {
                NineGridViewWrapper imageView = (NineGridViewWrapper) child;
                imageView.setIsVideo(true);
                isVedio = true;
            } else {
                isVedio = false;
            }
        ...
        boolean get = false;
        for (int i = 0, size = adapter.getImageInfo().size(); i < size; i++) {
            View child = getChildAt(i);
            if (child instanceof NineGridViewWrapper) {
                NineGridViewWrapper imageView = (NineGridViewWrapper) child;
                if (adapter.getImageInfo().get(i).url.endsWith(".gif")
                        || adapter.getImageInfo().get(i).url.endsWith(".GIF")) {
                    imageView.setIsGif(true);
                } else {
                    imageView.setIsGif(false);
                }
            }

            if (child instanceof NineGridViewWrapper) {
                NineGridViewWrapper imageView = (NineGridViewWrapper) child;
                if (adapter.getImageInfo().get(0).isVideo) {
                    imageView.setIsVideo(true);
                    isVedio = true;
                    get = true;
                } else {
        ...
        if (position < imageViews.size()) {
            ...
            imageView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    mAdapter.onImageItemOnClick(getContext(), position, mAdapter.getImageInfo());
                }
            });
